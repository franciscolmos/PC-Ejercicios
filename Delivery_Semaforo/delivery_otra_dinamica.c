...
int main(){
    ...
    s6 = sem_open("/semDelivery", O_CREAT, O_RDWR, 0);
    ...
    Delivery * delivery = inicializarDelivery(s5, s6, s3);
    ...
    borrarSemaforo(s1, s2, s3, s4, s5, s6);
...
void consultarPedidos(Delivery * delivery) {
    ...
    error = sem_trywait(delivery->semaforo2); //VERIFICA SI TIENE DINERO PARA ENTREGAR
    if(!error) {
        entregarDinero(delivery);
    }
}
...
void entregarDinero(Delivery * delivery) {
    int error = 0;
